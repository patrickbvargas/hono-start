...
| FR-DATA-07 | Business-entity form option queries (dropdowns, selects, autocompletes) must only return records where `isActive = true` and `deletedAt IS NULL`; lookup-table option queries must return all rows, render inactive values as disabled, and use the lookup `value` as the client-side selection identity. |
...
These are system-defined reference values. Their `value` and `label` fields are immutable — they cannot be created, renamed, or deleted through the application UI. However, each lookup value carries an `isActive` boolean (default: `true`) that administrators can toggle to keep a specific option visible but disabled in form dropdowns without removing the record. Lookup-backed form fields and URL filters use the stable lookup `value` on the client, while the server resolves that value to relational ids for persistence and querying.
