...
The system SHALL treat `src/features/clients_v2` as the reference slice for feature structure, ownership boundaries, list behavior, option-query behavior, protected mutation flows, and route-facing public surface. Future feature slices SHALL compare any planned deviation against that reference slice before changing the workflow shape.

#### Scenario: Clients v2 serves as the reference slice
- **WHEN** the application prepares to implement a new feature slice or refactor an existing one
- **THEN** `src/features/clients_v2` SHALL be treated as the baseline example for feature structure, ownership boundaries, list behavior, option-query behavior, protected mutation flows, and route-facing public surface
...
- **THEN** the deviation SHALL be compared against `src/features/clients_v2` before it is accepted
