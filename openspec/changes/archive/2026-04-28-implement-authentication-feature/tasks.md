...
- [x] 1.1 Add provedor legado de auth dependency, validated auth environment variables, and bootstrap/auth configuration files needed by TanStack Start.
- [x] 1.2 Add Prisma models and migration(s) required for provedor legado de auth persistence and for linking authenticated identities back to existing employees.
...
- [x] 2.1 Replace mock-only `src/shared/session` readers with optional and required authenticated-session helpers backed by provedor legado de auth.
...
- [x] 3.2 Implement remember-me behavior, logout action, and safe invalid-credential feedback using provedor legado de auth session APIs.
