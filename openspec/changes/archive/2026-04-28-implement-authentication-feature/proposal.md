...
- Replace placeholder or implicit session reads with a shared session contract derived from the authenticated provedor legado de auth session on both server and client consumers.
...
- Likely affected dependencies and systems: provedor legado de auth configuration, persistence needed for session/account/recovery data, and delivery provider wiring for password-reset messages
