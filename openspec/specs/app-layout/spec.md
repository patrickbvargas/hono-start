...
`AppLayout` SHALL render as the top-level shell component inside the root route, wrapping `<Outlet />` (all page content). It SHALL contain the sidebar and the main content area side by side in a flex row.

#### Scenario: All authenticated pages render inside AppLayout
- **WHEN** a user navigates to any route
- **THEN** the page content is rendered inside the AppLayout main area alongside the sidebar
